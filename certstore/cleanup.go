	...
	for _, secretPath := range secrets {
		...
		for _, version := range versions {
			...
			if expires.Before(now) {
				...
				safeToDestroy, err := RevokeCertificateWithVerification(context.Background(), logger, issuerAcmeClient, []byte(data.Cert), data.Issuer, data.Owner, data.Domain, data.Name, &versionNumber)
				if err != nil {
